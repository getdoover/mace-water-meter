...
class MaceWaterMeterApplication(Application):
    config: MaceWaterMeterConfig  # not necessary, but helps your IDE provide autocomplete!

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

        self.started: float = time.time()
        self.ui: MaceWaterMeterUI = None
        self.state: MaceWaterMeterState = None

    async def setup(self):
        self.ui = MaceWaterMeterUI()
